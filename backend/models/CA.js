...
// Compound index to ensure one CA attempt per user per course
caSchema.index({ user: 1, course: 1 }, { unique: true });

// Virtual for percentage score
caSchema.virtual('percentage').get(function() {
  if (this.totalQuestions === 0) return 0;
  return Math.round((this.score / this.totalQuestions) * 100 * 100) / 100;
});

// Method to calculate score
caSchema.methods.calculateScore = function() {
...
// Static method to check if user has attempted CA for a course
caSchema.statics.hasAttempted = async function(userId, courseId) {
